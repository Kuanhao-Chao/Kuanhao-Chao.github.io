...
import { auditGenomePage } from './audit-genome-ui.mjs';
...
async function auditBrowserExtras(page, scope) {
  ...
  await page.locator('.gb-tools').evaluate((el) => { el.open = true; });
...
async function auditGenomeBrowser(browser, baseURL, scope) {
  // The standalone genome gate owns its navigation, export and responsive contract.
  // Keep the playground smoke wired to it so shared-controller regressions still block release.
  await auditGenomePage(browser, baseURL, { smoke: SMOKE, full: FULL });
  // Preserve the existing renderer-specific guards as well as the new interaction contract.
  const context = await browser.newContext({ baseURL, viewport: { width: 1440, height: 950 } });
  const page = await context.newPage();
  try {
    await page.goto(GENOME_ROUTE, { waitUntil: 'networkidle' });
    await page.waitForSelector('[data-gb-ready="1"]');
