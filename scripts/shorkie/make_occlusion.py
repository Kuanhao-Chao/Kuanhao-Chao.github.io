...
Single strand, unlike the mutagenesis pack. The paper defines no RC convention for occlusion, and
inventing one here would be a claim rather than a reproduction.
...
def main() -> int:
    ...
    for locus in loci["loci"]:
        ...
        def coverage(inp: np.ndarray) -> np.ndarray:
            """T0-averaged coverage per output bin -- one pass, all 896 bins."""
