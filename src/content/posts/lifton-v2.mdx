...
description: 'LiftOn 2.0 rebuilds our DNA-plus-protein genome-annotation lift-over tool for whole-genome scale: lower memory, fewer crashes, broader feature recovery, and a safer merge validated across a 29-dataset benchmark.'
...
abstract: >-
  ...
  protected by a 24-cell byte-identity regression matrix. Across the 27 benchmarks where both LiftOn
  versions ran to completion — a divergence ladder reaching all the way to very distant cross-species pairs
  like eudicot-to-monocot and mammal-to-fish — LiftOn 2.0 matched or exceeded v1.0.8 on 26 of 27 and matched
  or exceeded the best single-method baseline on 24 of 27, the only losses being a handful of the most
  divergent pairs. On full Arabidopsis and rice RefSeq genomes, it finished runs the previous release
  abandoned and recovered tens of thousands more coding transcripts.
date: 2026-06-17
category: summary
...
imageAlt: "LiftOn 2.0's accuracy across twenty-nine benchmarks: a five-tier divergence ladder showing LiftOn stays most accurate as species diverge, the per-dataset gain over the best single-method baseline growing with divergence, and a per-transcript comparison showing the new merge improves many genes and regresses almost none."
references:
...
I evaluated LiftOn 2.0 on a 29-dataset benchmark spanning a five-tier divergence ladder: same-species assembly-to-assembly lift-overs, moderate cross-species pairs, close cross-species pairs, distant cross-species pairs, and a new very-distant tier (eudicot-to-monocot, mammal-to-fish, fly-to-mosquito, and budding-to-fission yeast) where genome-wide DNA synteny has all but disappeared. The comparison includes four tools: Liftoff alone, miniprot alone, LiftOn v1.0.8, and LiftOn 2.0. Every output is re-scored by the same evaluator rather than by each tool's own reporting. For coding transcripts, the evaluator translates the lifted CDS, aligns it to the reference protein, and computes mean protein identity.

Two full genomes from v1.0.8 crashed partway through, so they are not used for the head-to-head accuracy comparison. Across the 27 datasets where both LiftOn versions completed, LiftOn 2.0 matched or exceeded v1.0.8 on 26 of 27. It also matched or exceeded the better single-method baseline, Liftoff or miniprot, on 24 of 27. The three exceptions are all among the most divergent pairs: human-to-mouse (miniprot higher by 0.00931 mean protein identity), rice-to-sorghum (miniprot higher by 0.00644), and budding-to-fission yeast (Liftoff higher, but only on the four transcripts of 767 it manages to recover — a small-sample artifact). That is the expected boundary condition: combining DNA and protein helps most when both signals still contain useful information; when the DNA signal is very degraded, protein alone can be the cleaner evidence.

<ZoomFigure src={figAccuracy} alt="Three-panel accuracy figure. (A) A line plot of mean protein identity versus divergence class for Liftoff, miniprot, LiftOn v1.0.8, and LiftOn 2.0; all decline as species diverge, with LiftOn 2.0 highest through most of the ladder and the gap to the baselines widening. (B) A bar chart of LiftOn 2.0 minus the best single baseline for each benchmark, colored by divergence class, positive almost everywhere and largest on the most distant pairs, with the few negatives (human to mouse, rice to sorghum, and budding to fission yeast) annotated. (C) A grouped bar chart of transcripts improved versus regressed relative to v1.0.8 on the datasets that change, with improvements far outnumbering regressions." variant="wide">
  <strong>Figure 1.</strong> Accuracy across the divergence ladder. LiftOn 2.0 is strongest where DNA-only and protein-only evidence are both imperfect but still informative. It matches or exceeds the best single-method baseline on 24 of 27 completed benchmarks; the few non-wins are the most divergent pairs, where a single-method baseline edges ahead.
</ZoomFigure>
...
The new very-distant tier sharpens this picture. As genomes diverge to the point where DNA synteny is essentially gone — Arabidopsis to rice, human to zebrafish, fruit fly to mosquito — Liftoff's DNA-only model collapses to about 0.29 mean protein identity, while LiftOn 2.0 stays highest at about 0.54, because the merge falls back on the protein evidence that still survives. The pattern even holds on whole genomes: on full Arabidopsis-to-rice and full human-to-zebrafish, LiftOn 2.0 posts the highest per-transcript accuracy of all four tools, roughly 0.04 and 0.02 above the best single method. The honest counterpart is recall: at that distance miniprot's unconstrained protein search recovers far more transcripts (about 77 to 84 percent) than either LiftOn version (about 5 to 17 percent), so LiftOn's edge here is accuracy per recovered transcript, not coverage. Both LiftOn versions recover essentially the same set on these genomes, so this is a clean read on per-transcript quality rather than anything that changed in the new release.

Output validity also improves where scale stresses the pipeline. Re-running every output through one GFF3 validator, LiftOn 2.0 is at or below v1.0.8 on 22 of the 23 subset benchmarks and is strictly cleaner on all four completed full genomes. The main caveat is small but real: on the *S. cerevisiae* to *S. paradoxus* benchmark, the validator count increases from 10 to 11 errors. That single regression is worth keeping visible because a lift-over tool is only useful if its accuracy claims and its failure modes are both explicit.
