...
import SangerTrimming from '../../components/SangerTrimming.astro';
...
**A few lines, but explicit underneath.** That single `SangerAlignment` call does a sequence of concrete operations. It reads ABIF or FASTA inputs, groups reads into contigs by filename patterns or a CSV file, trims reads with either the modified Mott algorithm implemented in Phred (per-base sequencing quality scores) or the sliding-window approach implemented in Trimmomatic, both under one adjustable trimming cutoff, assembles forward and reverse reads into consensus contigs, and aligns the contigs together. The paper demonstrates this on eight annelid (*Allolobophora chlorotica*) samples, with one forward and one reverse ABIF read per sample.
...
Trimming is the one step where the two algorithms can hand you different reads from the same trace, so it is worth watching them disagree.
