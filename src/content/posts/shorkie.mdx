...
references:
  ...
  - text: 'Rhee, H. S. and Pugh, B. F. Comprehensive genome-wide protein-DNA interactions detected at single-nucleotide resolution. Cell (2011).'
    doi: 'https://doi.org/10.1016/j.cell.2011.11.013'
  ...
  - text: 'Renganaath, K. et al. Systematic identification of cis-regulatory variants that cause gene expression differences in a yeast cross. eLife (2020).'
    doi: 'https://doi.org/10.7554/eLife.62669'
---
...
> **🔬 Interactive In-Browser Lab** — Explore the published Shorkie checkpoints directly in your browser on [**Shorkie Lab**](/shorkie-lab/). Run live multi-method feature attributions (Saliency, Input × Gradient, SmoothGrad, Integrated Gradients, ISM), layer tracebacks, and occlusion maps on [**Shorkie Expression**](/shorkie-lab/shorkie/), and compute zero-shot sequence constraint and information content on [**Shorkie_LM**](/shorkie-lab/shorkie_lm/).
...
That is what happens at *RPL26A*. The language model and fine-tuned model both highlight the Fhl1 and Rap1 binding sites and the 5′ splice donor. The no-pretraining control, `Shorkie_Random_Init`, does not recover the same motif structure. You can explore the *RPL26A* locus interactively with both models on [Shorkie Lab](/shorkie-lab/).
...
<ZoomFigure src={figRpl26a} alt="Shorkie importance-score logos for the RPL26A locus, highlighting the Fhl1 and Rap1 transcription-factor motifs and the 5′ splice donor, above a no-pretraining control track." variant="wide">
  <strong>Figure 4.</strong> Reading the grammar back out, one gene at a time. At the ribosomal-protein gene <em>RPL26A</em>, two independent lenses agree: the pretrained language model's sequence-predictability signal (Shorkie LM) and <em>in silico</em> mutagenesis on the fine-tuned model (Shorkie ISM) both highlight the Fhl1 and Rap1 transcription-factor sites and the 5′ splice donor, matching the reference annotation at the bottom. The <em>Shorkie_Random_Init</em> track, an identical model without pretraining, does not recover the same motif-specific signal.
</ZoomFigure>

## Variants, dynamics, and the hard caveat

A useful regulatory model should say something about variants. We tested Shorkie on two association-based yeast *cis*-eQTL datasets (variants statistically linked to differences in nearby gene expression) and an [MPRA-validated benchmark of causal cis-regulatory variants (Renganaath et al., 2020)](https://doi.org/10.7554/eLife.62669). Across those three evaluations, Shorkie separated positive variants from matched controls better than the tested DREAM-challenge models, its randomly initialized counterpart, and the pretrained language model alone. The comparison shows that pretraining alone is not sufficient; fine-tuning on measured regulatory outputs adds task-specific signal.
...
*Read the [preprint on bioRxiv](https://doi.org/10.1101/2025.09.19.677475), browse the [code](https://github.com/calico/shorkie-paper), read the [documentation](https://khchao.com/shorkie/), explore the [data catalogue](https://khchao.com/shorkie/content/data_resources.html), try the models in-browser on [Shorkie Lab](/shorkie-lab/), or watch the [talk](https://youtu.be/MvpYQYQvZ0U). [The latest slides](https://storage.googleapis.com/storage.khchao.com/slides/ProbGen2026_0325.pdf) are also available. Shorkie was built with Majed Mohamed Magzoub, Emily Stoops, Sean R. Hackett, Johannes Linder, and David R. Kelley at Calico Life Sciences.*
