...
If you want to test whether a model can learn the rules of gene regulation, budding yeast (*Saccharomyces cerevisiae*) is almost the perfect laboratory. It is the premier model organism for eukaryotic gene regulation: roughly 7,000 genes controlled by hundreds of transcription factors, decades of careful experiments mapping who binds where, and a genome compact enough — about 12 megabases — to model end to end.

But that compactness cuts both ways. A 12 Mb genome simply doesn't contain enough examples to train a large deep-learning model from scratch. Sequence-to-expression models that work beautifully on the sprawling mammalian genome tend to *overfit* on yeast: with too few genes to learn from, they memorize instead of generalizing. So we had a model organism that was ideal in every respect except the one that matters most for deep learning — data.

## The bet: borrow power from evolution

Our idea was to get the missing data from somewhere yeast can't provide on its own: its relatives. Across the fungal kingdom, evolution has run essentially the same regulatory experiment millions of times over. The sequences that *matter* for regulation tend to be conserved; the ones that don't drift freely. A model that reads many fungal genomes should be able to pick up that conserved "grammar" — and carry it back to yeast.

This is the same self-supervised idea behind large language models, applied to DNA: instead of predicting the next word, the model learns to fill in masked stretches of genome, and in doing so internalizes which patterns are meaningful. I built Shorkie with the Kelley Lab at Calico, where we could pair this approach with something most groups don't have — the ability to *generate new data* to fine-tune on. Calico produced high-resolution RNA-seq time courses (the Induction Dynamics gene Expression Atlas, or IDEA) using miniaturized chemostats — "ministats" — that let us watch yeast genes respond to a transcription-factor induction minute by minute.
