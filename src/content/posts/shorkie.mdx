...
A useful regulatory model should say something about variants. We tested Shorkie on two association-based yeast *cis*-eQTL datasets and an [MPRA-validated benchmark of causal cis-regulatory variants (Renganaath et al., 2020)](https://doi.org/10.7554/eLife.62669). Across those three evaluations, Shorkie separated positive variants from matched controls better than the tested DREAM-challenge models, its randomly initialized counterpart, and the pretrained language model alone. The comparison shows that pretraining alone is not sufficient; fine-tuning on measured regulatory outputs adds task-specific signal.
