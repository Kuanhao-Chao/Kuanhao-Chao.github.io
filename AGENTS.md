...
- **`scripts/check-jsx-spacing.py`** finds swallowed spaces in `.astro` SOURCE. A newline between prose and an inline tag *or a JSX expression* is deleted by JSX rather than collapsed to a space, so `across the\n{nLoci} windows` renders "across the23 windows" — invisible in the source, and it survives `astro check`, the test suite and every static audit. `audit:playground` runs it over the three `/shorkie-lab/` routes before it opens a browser; run it by hand on any page you have been editing (`python3 scripts/check-jsx-spacing.py $(find src -name '*.astro')`). It found three live rendering bugs on its first widened run.
- **`scripts/shorkie/` is offline model tooling** — it turns the released Keras checkpoint into the ONNX and JSON that `/shorkie-lab/shorkie/` (interpretability, per locus) and `/shorkie-lab/genome/` (the genome browser) load. The route was `/variant-playground/`, which now redirects. CI never runs it and it needs `h5py numpy torch onnx onnxruntime` (plus `pyBigWig` for the optional measured-coverage overlay). Its outputs are committed; `scripts/shorkie/_scratch/` is not. `scripts/shorkie/README.md` holds the verification chain, and no change to `shorkie_torch.py` ships without re-running all of it.
